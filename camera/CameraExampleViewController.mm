...
static NSString* model_file_name = @"optimized_graph";  // tensorflow_inception_graph
...
static NSString* labels_file_name = @"retrained_labels"; //  imagenet_comp_graph_label_strings
...
const std::string input_layer_name = "Mul";  // gj was input
const std::string output_layer_name = "final_result"; // gj  was softmax1
...
- (void)observeValueForKeyPath:(NSString *)keyPath
  ...
  if (context == AVCaptureStillImageIsCapturingStillImageContext) {
    ...
    if (isCapturingStillImage) {
      ...
      [flashView setBackgroundColor:[UIColor redColor]];  // gj whiteColor
  ...
  if ([session isRunning]) {
    ...
    [UIView animateWithDuration:.2f
      ...
      for (NSDictionary *entry in sortedLabels) {
          ...
          NSLog(@"%@", label);
          NSLog(@"%@", valueObject);
          float val = [valueObject floatValue];
          if(val < 0.5) break;
          
          if([label isEqual: @"bus stop"]) {
              NSLog(@"busstop found.............");
...
//              [pasteboard setData:imageData forPasteboardType:(NSString *)kUTTypePNG];
              ...
              [[UIPasteboard generalPasteboard] setImage:uiImage];
              
              [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"njbus://"]];
              
          } else if([label  isEqual: @"barcode"]) {
              NSLog(@"barcode found");
              
              [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"com.amazon.mobile.shopping://"]];
              
          
          }  else if ([label  isEqual: @"receipt"]) {
              NSLog(@"receipt found");
              [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"twtOpenApp.IQBoxy://"]];
...
didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer
    ...
    // gj TODO org: CVPixelBufferRef = ..    CVPixelBufferRef pixelBuffer = CMSampleBufferGetImageBuffer(sampleBuffer);
...
- (void)runCNNOnFrame:(CVPixelBufferRef)pixelBuffer {
  ...
  if (tf_session.get()) {
    ...
    if (!run_status.ok()) {
      ...
      // NSMutableDictionary *newValues = [NSMutableDictionary dictionary];  //gj
      newValues = [NSMutableDictionary dictionary];
...
- (void)setPredictionValues:(NSDictionary *)newValues {
  ...
  NSSortDescriptor *sort =
    ...
    // gj TODO NSArray *sortedLabels
  sortedLabels = [candidateLabels
